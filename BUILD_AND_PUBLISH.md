...
脚本在提交发布说明到平台时，会自动追加当前 Git 仓库 `HEAD` 的 commit id。

默认效果：

```text
v2026.4.8 release
commit: 51226e5a53f512bae2b63abb721c12747fc3832f
```

如果手动传了 `--release-notes`，脚本也会继续在你传入的内容后面追加同一个 commit id。

示例：

```bash
python3 build_and_publish.py --arch mac --release-notes "macOS Apple Silicon build"
```
...
如果当前目录不是一个可读取 Git 信息的仓库，脚本会跳过 commit id 注入，但不会阻塞发布流程。
...
- `--release-notes`：版本说明
  即使手动指定，脚本也会自动追加当前仓库的最新 commit id
